...
if (perms) {
    ...
    try {
      ...
      tbody.innerHTML = res.data
        ...
        <tr>
          ...
          <td class="text-sm">${listOrAll(r.module_access || [])}</td>
          <td class="text-sm">${listOrAll(r.actions || [])}</td>
          <td>${r.user_count}</td>
          <td>
  ...
  function openRoleModal(role) {
    const isEdit = !!role;
    const selectedModules = role?.module_access || [];
    const selectedActions = role?.actions || [];
